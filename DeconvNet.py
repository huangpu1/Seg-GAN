...
class DeconvNet:
    def __init__(self, use_cpu=False, checkpoint_dir="./checkpoints/"):
        ...
        config = tf.ConfigProto(allow_soft_placement=10)
    ...
    def resotre_session(self):
        global_step = 0
        import os
        if not os.path.exists(self.checkpoint_dir):
            raise IOError(self.checkpoint_dir + "dose not exist")
        else:
    ...
    def train(self, train_stage=1, training_step=1000, restore_session=False, learning_rate=1e-6):
        ...
        for i in range(step_start, step_start + training_step):
            import random
            import cv2, time
            import numpy as np
    ...
    def deconv_layer(self, x, W_Shape, b_shape, name, padding='SAME'):
        ...
        return tf.nn.conv2d_transpose(x, W, out_shape, [1, 1, 1, 1], padding) + b
